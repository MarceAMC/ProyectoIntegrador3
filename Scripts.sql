...
-- ---------------------
-- Clientes que hicieron compras en 2019 y 2020
SELECT cliente.id, cliente.nombre_y_apellido
FROM cliente
INNER JOIN venta ON cliente.id = venta.idcliente
WHERE venta.fecha BETWEEN '2019-01-01' AND '2020-12-31';
-- Porcentaje de ventas que representa cada mes, dentro del total de año 2018.
SELECT
  MONTH(fecha) AS mes,
  SUM(precio * cantidad) AS total_ventas,
  (SUM(precio * cantidad) OVER (PARTITION BY MONTH(fecha)) / SUM(precio * cantidad)) * 100 AS porcentaje
FROM venta
